...
$(document).ready(function(){
    ...
    $('#save_btn').on("click",function(){
        if ( $('input:radio[name=drawwhat]:checked').val() == 'line')	{	// add a new line 
            ...
            if (path_line.length > 0)	{
                ...
                $('#map_Lineid').val(map_polylines.indexOf(map_line));
            ...
            if (path_poly.length > 0)	{
                ...
                $('#map_Polygonid').val(map_polygons.indexOf(map_polygon));
	...
	if(window.location.pathname == "/central-taxi/super-admin/add-zone-administrator.php")
            {
                    $('div#basic_options .radio').hide();
                   $('div#basic_buttons #add_btn').hide();
            }
        }	
        else if ( $('input:radio[name=drawwhat]:checked').val() == 'rectangle')	{
            ...
            if (path_rect.toString().length > 0)	{
                ...
                $('#map_Rectangleid').val(map_rectangles.indexOf(map_rectangle));
            ...
            if (path_circle.length > 0)	{
                ...
                $('#map_Circleid').val(map_circles.indexOf(map_circle));
    ...
    GMaps.on('marker_added', map, function(marker) {
        ...
        //$('#allmarks').append('<div id="marker_'+map.markers.indexOf(marker)+'_div"><input name="marker_title[]" type="text" id="marker_'+map.markers.indexOf(marker)+'" value="Marker #'+map.markers.indexOf(marker)+'"><input name="marker_coords[]" id="marker_coords_'+map.markers.indexOf(marker)+'" type="text" value="(' + marker.getPosition().lat() + ',' + marker.getPosition().lng() + ')">&nbsp;&nbsp;<button type="button" class="btn btn-info btn-xs pan-to-marker" id="marker_'+map.markers.indexOf(marker)+'_cnr" data-marker-lat="' + marker.getPosition().lat() + '" data-marker-lng="' + marker.getPosition().lng() + '">Show marker</button>&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-xs" id="marker_'+map.markers.indexOf(marker)+'_rmv" href="#" onClick="deleteMarker('+map.markers.indexOf(marker)+');">Eliminar</button></div>');
        $('#allmarks').append('<div id="marker_'+map.markers.indexOf(marker)+'_div"><input name="marker_title[]" type="text" id="marker_'+map.markers.indexOf(marker)+'" value="'+marker.getTitle()+'"><input name="marker_coords[]" id="marker_coords_'+map.markers.indexOf(marker)+'" type="text" value="(' + marker.getPosition().lat() + ',' + marker.getPosition().lng() + ')"><input name="marker_icon[]" id="marker_icon_'+map.markers.indexOf(marker)+'" type="hidden" value="' + marker.getIcon() + '">&nbsp;&nbsp;<button type="button" class="btn btn-info btn-xs pan-to-marker" id="marker_'+map.markers.indexOf(marker)+'_cnr" data-marker-lat="' + marker.getPosition().lat() + '" data-marker-lng="' + marker.getPosition().lng() + '">Show marker</button>&nbsp;&nbsp;<button id="marker_'+map.markers.indexOf(marker)+'_chm" type="button" class="btn btn-xs btn-primary" data-toggle="popover" title="Select Icon" data-content="" data-placement="top">Change icon</button>&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-xs" id="marker_'+map.markers.indexOf(marker)+'_rmv" href="#" onClick="deleteMarker('+map.markers.indexOf(marker)+');">Remove</button></div>');


        $('#marker_'+map.markers.indexOf(marker)+'_chm').on('show.bs.popover', function () {
          $('#what_todo').html(map.markers.indexOf(marker));
        })	
        $('#marker_'+map.markers.indexOf(marker)+'_chm').on('hidden.bs.popover', function () {
          $('#what_todo').html('');
        })
        $('#marker_'+map.markers.indexOf(marker)+'_chm').popover({
            html : true, 
            content: function() {
...
function writeOnHelpbox(objectclicked) {
    if (objectclicked == 'drawwhat_markers')	{
        ...
        $("#helpbox").text("Click the button 'Add polygon' to draw a polygon");	
    }
...
// delete a polygon and its input boxes -- delete by nj  // 
function deletePolygon_sub_zone_ByNj(index,cordinatedIDDD) {
    
    var r = confirm("Do you want to delete this sub zone?");
    if (r == true) {
        $.post('../super-admin/getData.php',{mode:'cmVtb3ZlX2NvcmRpbmF0ZWRzX29mX2RyaXZlcnM=',a:cordinatedIDDD},function(response){
        $('#erorrMsg').html('<div class="alert alert-danger"><button class="close loginError" data-dismiss="alert" type="button">x</button>Remove sub zone successfully</div>');
    });
...
function deletePolygon(index) {
    ...
    if(window.location.pathname == "/central-taxi/super-admin/add-zone-administrator.php")
    {
        $('div#basic_options .radio').show();
        $('div#basic_buttons #add_btn').show();
    }	
} 
...
function loadMarker(lat, lng, title, marker_icon, draggable_state) {
    var index = map.markers.length;
    var newmarker = map.addMarker({
        lat: lat,
        lng: lng,
...
// draw a polygon by Nj from a given path 
function loadPolygonByNjj(path,title,cordinateId) {
    var pindex = map_polygons.length;
    map_polygon = map.drawPolygon({
        paths: path, // pre-defined polygon shape
        index: pindex,
    ...
    if(window.location.pathname == "/central-taxi/taxi-company/wrok_zone_create.php")
    {
       if(cordinateId == 'Main_Zone')
        {
        ///console.log(window.location.pathname);    
        } 
        else
        {
            ...
            $('#allmarks').append('<div id="poly_'+map_polygons.indexOf(map_polygon)+'_div"><input type="hidden" name="cordinateId_Main_Zone[]" value="'+cordinateId+'"/><input name="poly_title[]" type="text" id="poly_'+map_polygons.indexOf(map_polygon)+'" value="'+title+'"><input name="poly_coords[]" id="poly_coords_'+map_polygons.indexOf(map_polygon)+'" type="text" value="' + path + '">&nbsp;&nbsp;<button type="button" class="btn btn-primary btn-xs" id="poly_'+map_polygons.indexOf(map_polygon)+'_edit"  onClick="editPolygon('+map_polygons.indexOf(map_polygon)+');" >Edit</button>&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-xs" id="poly_'+map_polygons.indexOf(map_polygon)+'_delete" onClick="deletePolygon_sub_zone_ByNj('+map_polygons.indexOf(map_polygon)+','+cordinateId+');" >Remove</button></div>');  // cordinatedIDDD deletePolygon_sub_zone_ByNj cordinateId   
        }
    }
    else
    {
...
// draw a polygon by Nj from a given path for forbidden zone
function loadPolygonForbiddenByNjj(path,title,cordinateId) {
    var pindex = map_polygons.length;
    map_polygon = map.drawPolygon({
        paths: path, // pre-defined polygon shape
        index: pindex,
...
// draw a polygon For Main Zone by Nj from a given path
function loadPolygonMainZoneByNjj(path,title,cordinateId) {
    var pindex = map_polygons.length;
    map_polygon = map.drawPolygon({
        paths: path, // pre-defined polygon shape
        index: pindex,
...
function createOverlay(lat, lng, title, shape_area)	{
    ...
    var overl = map.drawOverlay({
        lat: lat,
        lng: lng,
        content: '<div class="overlay">'+title+shape_area_txt+'</div>'
    });		
...
function doMap(map_id, what) {
    var action_file;
    if (what=='delete')	{
		action_file = 'edit_zone.php?p=maps';
    }
    else  {
		action_file = 'edit_zone.php?p=map_edit&map_id='+map_id;
    }
