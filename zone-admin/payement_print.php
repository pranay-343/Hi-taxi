        ...
        <div class="c-acc-status mg0">
          <h2 class="txt-style-3">View Message </h2>
            ...
            <?php 
                ...
                $str="select * from manage_master_amount where 1 and company_id='$id' ORDER BY id DESC";
                $res=mysql_query($str);
                $row=mysql_fetch_array($res);

                //if($row['user_type'] == 'driver'){
                    
                    $data_com = mysql_fetch_array(mysql_query("SELECT * FROM login WHERE id = '".$row['company_id']."'"));
                    $name = $data_com['name'];
               // }
            ?>
          <div class="row bts">
              ...
              <div class="c-acc-status bst mgmin" id="content">
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" border="0" class="ctabel1">
                      ...
                      <tr> 
                          ...
                          <td class="tab-txt1"><?php echo $row['amount'];?></td>
                          <td class="tab-txt1"><?php echo date('Y-m-d', strtotime($row['start_date'])).'-'.$row['end_date_time'];?></td>
