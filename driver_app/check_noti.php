...
function iosDevice($deviceToken,$message, $types,$devlopment,$devlopment1)
{
	//print_r($devlopment1);
	if($types=='7')
	{
...
{	
	...
	if(!empty($data['0']))
	{
	  ...
	  $status='0';
	  $str="SELECT `driver`.id,`driver`.company_id,`taxicompany`.web_user_id,`taxicompany`.name  FROM driver
	...
	{	
				...
				{
					if($row1['name'] == $row['company_name'] || $row['company_name'] == 'ANY COMPANY')
					{
...
function acceptTrip()
{
	...
	if(!empty($data['0']))
	{
		...
		if($status=='0')
		{
		$q2="UPDATE trip SET status='200',driver_id='".$driverId."',trip_mode='waiting',trip_acceptTime='".$dateTime."' where id='".$tripId."'";
		...
		if($row1['device_type'] == 'IOS')
		{
	        ...
	        iosDevice($deviceToken, $string, $types,$devlopment,$devlopment1);
		}
		else
		{
		$registatoin_ids = array($gcm_regid);
...
function locationUpdate()
{
    ...
    $id=mysql_insert_id() or die(mysql_error());
  	...
  	{
    $q="INSERT INTO `trip_log`(`longitude`,`latitude`,`driver_id`,`date`,`zone_type`,`anroid_id`) VALUES ('$longitude','$latitude','$driver_id','$date','$inout','$android_id')";
    $res=mysql_query($q) or die(mysql_error());
    $id=mysql_insert_id() or die(mysql_error());
    ...
    $q1="UPDATE `trip_log` SET longitude='".$longitude."',latitude='".$latitude."',driver_id='".$driver_id."',date='".$date."',status='".$row1['status']."',zone_type='".$inout."',anroid_id='".$android_id."' where id='".$id."' ";
    $res2=mysql_query($q1) or die(mysql_error());
    $contents=array();
	if(mysql_affected_rows() > 0)
	 { 
...
function getAdminNotification()
{
	...
	{ 
		...
		$str="SELECT `send_message`.driver_name,`send_message`.message,`driver`.id,`driver`.name,`send_message`.heading,`send_message`.added_on FROM `send_message` LEFT JOIN `driver` ON `send_message`.driver_name=`driver`.id where `driver`.id='".$DriverId."' or `send_message`.driver_name ='0'";
		$res=mysql_query($str);
		if(mysql_affected_rows()>0)
		{
			while($row=mysql_fetch_array($res))
