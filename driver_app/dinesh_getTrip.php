...
function getTrip()
{
    ...
    if(!empty($data['0']))
    {
      ...
      $str200="SELECT `driver`.status,`driver`.id,`driver`.company_id,`driver`.zone_id,`driver`.zone_primary_id,`taxicompany`.web_user_id,`taxicompany`.name,`driver`.device_id  FROM `driver`
          LEFT JOIN `taxicompany` on `driver`.company_id = `taxicompany`.web_user_id where `driver`.id='$driverId1'";
      $res200=mysql_query($str200);
      $row200=mysql_fetch_assoc($res200);
      $row200['name'];
      $gcm_regid = $row200['device_id'];
     ...
     if($row200['zone_id'] == '0')
     {
      $strCordinated="SELECT `zone_cordinater`.cordinated,`zone_cordinater`.id,`zone_cordinater`.zone_area_id,`taxicompany`.id ,`taxicompany`.zone_area_id_sess,`taxicompany`.name,`taxicompany`.web_user_id from `zone_cordinater`
          LEFT JOIN `taxicompany` ON `zone_cordinater`.zone_area_id=`taxicompany`.zone_area_id_sess where `zone_cordinater`.zone_area_id='".$row200['zone_primary_id']."' and `taxicompany`.web_user_id='".$row200['company_id']."'";
       $resultCordinated=mysql_query($strCordinated);
       // if(mysql_num_rows($resultCordinated) > 0)
       // {
            $rowCordinated = mysql_fetch_assoc($resultCordinated);
            $data1= $rowCordinated['cordinated'];
          //  print_r($data1)."mine" ;
            $dinesh=explode(",", $data1);
            //print_r($dinesh)."hello";
           ...
           for ($b=0; $b < count($dinesh); $b++) 
              ...
              $val='';
        $str1000="select source_latitude,source_longitude,id,tripdatetime from trip where status='0' and source_latitude != '' and source_longitude != '' and tripdatetime BETWEEN '$todayDate' and '$todayDate1' order by id DESC";
        ...
        $res1000=mysql_query($str1000);
        
        while($row1000=mysql_fetch_array($res1000))
        {
        $latitude = $row1000['source_latitude'];
        $longitude = $row1000['source_longitude'];
      ...
      for($i=0;$i<3;$i++)
      {
        //echo $i."dinesh".$row1000['id'];
      //  print_r($val[$i])."dinesh";
        $vertices_y = $odd; // y-coordinates of the vertices of the polygon
        $points_polygon = count($vertices_x) - $i;  // number vertices - zero-based array
        ...
        if (is_in_polygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y))
        {
          ...
          $myData="SELECT `trip`.sendNotification,`trip`.company_name,`trip`.id,`trip`.source_address,`trip`.source_city,`trip`.source_state,`trip`.source_landmark,`trip`.source_latitude,`trip`.source_longitude,`trip`.destination_address,`trip`.destination_city,`trip`.destination_state,`trip`.destination_landmark,`trip`.destination_latitude,`trip`.destination_longitude,`trip`.travelTime,`trip`.tripdatetime,`trip`.agreement,`trip`.trip_distance,`trip`.price_perkm,`users`.name,`users`.contact_no,`trip`.trip_type,`trip`.trip_ammount from `trip`
           LEFT JOIN `users` ON `trip`.customer_id=`users`.id
            where `trip`.id='".$row1000['id']."'";
            $resMyData=mysql_query($myData);


            $uniques_nj  = array(); // code by nj
            while($rowMyData=mysql_fetch_assoc($resMyData))
            {
            //  echo $rowMyData['id']."dinesh";

              


              if($rowMyData['trip_type'] == 'userapp')  
              {
              if($row200['name'] == $rowMyData['company_name'] || $rowMyData['company_name'] == 'ANY COMPANY')
              {
               // echo $rowMyData['id']."dinesh";
                $uniques[] = $rowMyData['id']; // code by nj
                $cnt_array = array_count_values($uniques);  // code by nj
                $res = array();  // code by nj
                foreach($cnt_array as $key=>$val)
                {  // code by nj
                   if($val == 1)
                   {  // code by nj
                      $contents100[]=$rowMyData;  // code by nj
                   }  // code by nj
                }  // code by nj
                  
                  if($rowMyData['sendNotification'] == '0')
                  {
                  $registatoin_ids = array($gcm_regid);
                  $message = array("newTrip" => "You have got a new trip");
                  send_notification($registatoin_ids, $message);

                  $send_notification=mysql_query("update `trip` set sendNotification='1' where id='".$rowMyData['id']."'");
                  }
              } // end if looop
            } // end if for trip type
       
            else
            {
                  // $contents100[]=$rowMyData;

                  $uniques[] = $rowMyData['id']; // code by nj
                  $cnt_array = array_count_values($uniques);  // code by nj
                  $res = array();  // code by nj
                  foreach($cnt_array as $key=>$val){  // code by nj
                     if($val == 1){  // code by nj
                        $contents100[]=$rowMyData;  // code by nj
                     }  // code by nj
                  }  // code by nj
         ...
         } // end if loop for is_in_polygon
       } // end while looop here for $row1000
      
        if($contents100 !="")
          {
            $finalData=$contents100;
           // $finalData=array_unique($contents100, SORT_REGULAR);
          
           // $withoutDuplicates = array_unique(array_map("strtoupper", $contents100));
            $msg['message']='successfull';
            $msg['result']=$finalData;
  ...
  {
         $strCordinated6000 = "SELECT `zone_cordinater_driver`.cordinated,`zone_cordinater_driver`.id,`zone_cordinater_driver`.zone_area_dr_id,`taxicompany`.id ,`taxicompany`.zone_area_id_sess,`taxicompany`.name,`taxicompany`.web_user_id from `zone_cordinater_driver`
            LEFT JOIN `taxicompany` ON `zone_cordinater_driver`.zone_area_dr_id=`taxicompany`.zone_area_id_sess where `zone_cordinater_driver`.id='".$row200['zone_id']."' and `taxicompany`.web_user_id='".$row200['company_id']."'";
         $resultCordinated6000 =mysql_query($strCordinated6000);
         // if(mysql_num_rows($resultCordinated) > 0)
         // {
          $rowCordinated6000 = mysql_fetch_assoc($resultCordinated6000);
          $data6000= $rowCordinated6000['cordinated'];
         // print_r($data1)."mine" ;
          $dinesh6000=explode(",", $data6000);
          //print_r($dinesh)."hello";
          $dinesh6000= str_replace("(", "", $dinesh6000);
          $dinesh6000= str_replace(")", "", $dinesh6000);
          
         for ($b=0; $b < count($dinesh6000); $b++) 
            { 
              if($b == '0' || $b%2 == '0')
              {
                $even6000[] = $dinesh6000[$b];
              }
              else
              {
                $odd6000[] = $dinesh6000[$b];
              }
            } // end for loop here

             // $str1000="select source_latitude,source_longitude,id from trip where status='0' and source_latitude != '' and source_longitude != '' ";
           $str1000="select source_latitude,source_longitude,id,tripdatetime from trip where status='0' and source_latitude != '' and source_longitude != '' and tripdatetime BETWEEN '$todayDate' and '$todayDate1' order by id DESC";
            $res1000=mysql_query($str1000);
           ...
           for($i=0;$i<3;$i++)
           {
              $vertices_y = $odd6000; // y-coordinates of the vertices of the polygon
              //print_r($vertices_y);
              $points_polygon = count($vertices_x) - $val;  // number vertices - zero-based array
              $longitude_x = $latitude;  // x-coordinate of the point to test
              $latitude_y = $longitude;    // y-coordinate of the point to test
            if (is_in_polygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y))
            {
              $myData="SELECT `trip`.company_name,`trip`.id,`trip`.source_address,`trip`.source_city,`trip`.source_state,`trip`.source_landmark,`trip`.source_latitude,`trip`.source_longitude,`trip`.destination_address,`trip`.destination_city,`trip`.destination_state,`trip`.destination_landmark,`trip`.destination_latitude,`trip`.destination_longitude,`trip`.travelTime,`trip`.tripdatetime,`trip`.agreement,`trip`.trip_distance,`trip`.price_perkm,`users`.name,`users`.contact_no,`trip`.trip_type,`trip`.trip_ammount from `trip`
             LEFT JOIN `users` ON `trip`.customer_id=`users`.id
              where `trip`.id='".$row1000['id']."' order by `trip`.id DESC";
              $resMyData=mysql_query($myData);
            while($rowMyData=mysql_fetch_assoc($resMyData))
            {
               ...
               if($rowMyData['trip_type'] == 'userapp')  
                {
                if($row200['name'] == $rowMyData['company_name'] || $rowMyData['company_name'] == 'Cualquiera')
                {
               // echo $rowMyData['id']."dinesh";
                  $uniques[] = $rowMyData['id']; // code by nj
                  $cnt_array = array_count_values($uniques);  // code by nj
