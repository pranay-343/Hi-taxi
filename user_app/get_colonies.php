...
function companyName_2()
{
    ...
    $str="SELECT `zone_cordinater`.zone_type,`taxicompany`.id,`taxicompany`.zone_area_id_sess,`taxicompany`.name,`zone_cordinater`.zone_area_id,`zone_cordinater`.cordinated FROM `zone_cordinater`
	    ...
	    //         LEFT JOIN `taxicompany` ON `zone_cordinater`.zone_area_id=`taxicompany`.zone_area_id_sess where `zone_cordinater`.zone_type='polygon' ";	     
			...
			if(is_in_polygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y))
			{
				$str1="SELECT `zone_cordinater`.zone_type,`taxicompany`.id,`taxicompany`.zone_area_id_sess,`taxicompany`.name,`zone_cordinater`.zone_area_id,`zone_cordinater`.cordinated FROM `zone_cordinater`
	          LEFT JOIN `taxicompany` ON `zone_cordinater`.zone_area_id=`taxicompany`.zone_area_id_sess where `zone_cordinater`.zone_type='polygon' and `zone_cordinater`.zone_area_id='$zone_area_id'";
            $res1=mysql_query($str1);
            while($row1=mysql_fetch_array($res1))
            {
			 // echo "Is in polygon!"."</br>";
			   $companyName['companyName'][]=$row1['name'];
			   $companyName['id'][]=$row1['id'];
			   //print_r($companyName);
			 }
			 // echo "Is in polygon!"."</br>";
			   $companyName[]=$name;
